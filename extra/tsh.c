...
//TODO
    ...
    if( is_pid ){
        ...
        job_ref = getjobjid(jobs, atoi(&argv[1][1]));
        if(job_ref == NULL){
    ...
    //kill process and wait for it to be reaped by sig_child handler
...
void do_bgfg(char **argv) 
{
    ...
    if(is_jid){
       ...
       if(job_ref == NULL){
    	...
    	job_ref = getjobpid(jobs, (pid_t) atoi(argv[1]));
    	if(job_ref == NULL){
       	   printf("(%d): No such process\n", atoi(argv[1]));
           return;
       }
    ...
    if(strcmp(argv[0], "bg") == 0) {
        job_ref->state = BG;                                            // Change (FG > BG) or (ST ->  BG)
        printf("[%d] (%d) %s", job_ref->jid, job_ref->pid, job_ref->cmdline);
        safe_kill(-job_ref->pid, SIGCONT);                              // Send SIGCONT signal to entire group of the given job
    } else {
        job_ref->state = FG;                                            // Change (BG -> FG) or (ST -> FG)
        safe_kill(-job_ref->pid, SIGCONT);                              // Send SIGCONT signal to entire group of the given job
        waitfg(job_ref->pid);                                           // Wait for fg job to finish
    }
